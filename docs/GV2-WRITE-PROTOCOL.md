...
**Issue**: st-1x5 (LEG 1: Research GV2 Protocol)
**Convoy**: st-2cq (GV2 Group CRUD)
**Blocks**: LEG 2 (libsignal GroupsManager write ops), LEG 3 (presage Manager group APIs)
...
### roder/libsignal-service-rs (fork)

3 commits ahead of upstream (poll proto support + tests + .gitignore).
GroupsManager and operations.rs are **identical** to upstream -- no group write ops.
Only branch: `main` (feature/protocol-v8-polls-rebased was merged and deleted).

### roder/presage (fork)

5 commits ahead of upstream. Adds poll support (send_poll, vote_on_poll, terminate_poll).
No group creation/membership management.
Only branch: `main` (integration/protocol-v8-polls does not exist).
Points to `roder/libsignal-service-rs main` in Cargo.toml.

### LEG 2 Base Branch Correction

The convoy plan specifies `feature/protocol-v8-polls-rebased` as the base branch for LEG 2.
This branch **does not exist** -- it was merged into `main` and deleted.
**LEG 2 should branch from `roder/libsignal-service-rs main`.**

Similarly, `integration/protocol-v8-polls` on roder/presage does not exist.
**LEG 3 should branch from `roder/presage main`.**
